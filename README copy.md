...
## Multi-Format Document Support & Deduplication

### ✨ New Features

#### 1. Multi-Format Document Support
The ingestion pipeline now supports downloading and processing documents in multiple formats:
- **PDF** — via PyMuPDF (existing)
- **DOCX** — Microsoft Word documents via python-docx
- **HTML** — Web pages via BeautifulSoup
- **XML** — Structured XML documents via lxml
- **TXT** — Plain text files with automatic encoding detection

All formats are automatically extracted to text and processed through the same chunking and embedding pipeline.

#### 2. Intelligent Deduplication
To prevent redundant downloads and ensure efficient resource usage:
- **URL-based tracking** — Registry tracks all downloaded URLs
- **File hash deduplication** — Detects identical content by SHA-256 hash
- **Persistent storage** — Registry stored in `data/metadata/dedup_registry.json`
- **Automatic skipping** — Scrapers can check before downloading each URL
...
if not tracker.is_url_downloaded(url):
    download_file(url, path)
    tracker.mark_downloaded(url, path, doc_format="pdf")

# Get statistics
stats = tracker.get_statistics()
print(f"Downloaded {stats['total_unique_urls']} unique documents")
```
...
| Module | Purpose |
|---|---|
| `src/ingestion/document_handler.py` | Multi-format text extraction with format detection |
| `src/ingestion/deduplication_tracker.py` | Download tracking with URL & hash-based deduplication |

### 💾 Registry

The deduplication registry is stored in `data/metadata/dedup_registry.json` and tracks:
- **downloaded_urls**: URL → metadata (timestamp, path, hash, format)
- **downloaded_files**: SHA-256 hash → metadata (for content-based dedup)
