...
With deduplication tracking to prevent redundant downloads.
"""
...
async def run_mass_ingestion():
    ...
    logger.info(f"Initialized dedup tracker. Current stats: {dedup_tracker.get_statistics()}")
    ...
    try:
        ...
        logger.info("Constitution ingestion complete.")
    ...
    logger.info(f"Total unique URLs downloaded: {stats['total_unique_urls']}")
    logger.info(f"Total unique files (by hash): {stats['total_unique_files']}")
